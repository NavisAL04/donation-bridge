...
const CONFIG = {
  ...
  DISCORD_WEBHOOK:   process.env.DISCORD_WEBHOOK_URL, // Pakai webhook URL, bukan bot token
};

// ============================================================
// IN-MEMORY STORAGE
// Catatan: di Vercel serverless, memory RESET setiap cold start
// Tapi untuk polling Roblox ini sudah cukup karena:
// - Vercel keep warm selama ada request
// - Donasi langsung diproses saat webhook masuk
// ============================================================
const donations = [];
const sessions  = {};
...
// ============================================================
// DISCORD WEBHOOK (tidak perlu bot token!)
// Cukup pakai Webhook URL dari Discord channel
// ============================================================
      ...
      body: JSON.stringify({
        embeds: [{
          ...
          fields: [
            ...
            { name: '🎯 Platform', value: source,  inline: true },
...
app.get('/', (req, res) => {
  res.json({
    ...
    discord:        CONFIG.DISCORD_WEBHOOK ? 'webhook ready' : 'disabled',
  });
});
